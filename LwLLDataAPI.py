...
def create_session():
	...
	r = requests.post(f"{url}/auth/create_session", json={'session_name': 'testing', 'data_type': data_type, 'task_id': task_id}, headers=headers)
...
def deactivate_session(session_token,session="all"):
	...
	if session == "all":
		for current_session in set(all_active_sessions) - set([session_token]): # Deactivate all sessions apart from the current session
			print("Deactivation Session: " + current_session)
			r = requests.post(f"{url}/deactivate_session", json={'session_token': current_session}, headers=headers_active_session)
	else:
		if session not in all_sessions:
			print("ERROR: This session does not exist!")
		else:
			r = requests.post(f"{url}/deactivate_session", json={'session_token': session}, headers=headers_active_session)
